...
create_database_if_not_exists "post_db"
create_database_if_not_exists "stock_db"
create_database_if_not_exists "prediction_db"

# Auth Service tables
echo "Setting up auth_db tables..."
...
# Post Service tables
echo "Setting up post_db tables..."
...
echo "Setting up stock_db tables..."
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "stock_db" <<-EOSQL
    ...
    -- Insert sample stock data only if table is empty
...
echo "Setting up prediction_db tables..."
...
# User Service tables
echo "Setting up user_db tables..."
